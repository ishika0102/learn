...
def insert_documents(start, end):
    ...
    db = client["data_dump"]
    collection = db["test_collection_004"]
    ...
    collection.create_index([("DeviceName", ASCENDING)])  # Additional index for testing
    collection.create_index([("BelongToPriOrgId", ASCENDING)])  # Additional index for testing
    collection.create_index([("BelongToSubOrgId", ASCENDING)])  # Additional index for testing
    collection.create_index([("GroupId", ASCENDING)])  # Additional index for testing
    collection.create_index([("PolicyId", ASCENDING)])  # Additional index for testing
    collection.create_index([("NonCompilant", ASCENDING)])  # Additional index for testing
    collection.create_index([("Battery", ASCENDING)])  # Additional index for testing
    collection.create_index([("SignalStrength", ASCENDING)])  # Additional index for testing
    collection.create_index([("AuditDetails.CreatedOn", ASCENDING)])  # Additional index for testing
    collection.create_index([("AuditDetails.ModifiedOn", ASCENDING)])  # Additional index for testing
    collection.create_index([("NetworkType", ASCENDING)])  # Additional index for testing
    collection.create_index([("AuditDetails.CreatedBy", ASCENDING)])  # Additional index for testing
    collection.create_index([("AuditDetails.ModifiedBy", ASCENDING)])  # Additional index for testing
#     collection.create_index([("TimeStamp", ASCENDING)])  # Additional index for testing
...
if __name__ == "__main__":
    ...
    num_docs = 50000
    batch_size = 1000  # Adjust the batch size as needed for performance
